...
Создать социальную fashion-платформу с виртуальной примеркой, которая повышает конверсию в покупку, вовлечённость пользователей и снижает возвраты для брендов и маркетплейсов.
...
Бизнес-ограничения.
Ограниченные вычислительные ресурсы на старте, отсутствие собственного датасета большого объёма, необходимость быстрого MVP.

Ожидания от итерации.
Рабочий MVP виртуальной примерки с приемлемым качеством визуализации и базовой персонализацией.

Бизнес-процесс пилота.
Пользователь загружает фото или использует камеру → модель строит цифровой аватар → пользователь примеряет одежду → сохраняет / публикует образ → переходит к бренду.
...
Что закрываем в итерации.
- Базовая ML-модель виртуальной примерки
- Персонализация визуализации
- Инференс в браузере / через API

Что не закрываем.
- Фотореализм уровня AAA
- Массовая автоматическая генерация одежды

Качество кода и воспроизводимость.
Код модульный, воспроизводимый, обучение и инференс разделены, эксперименты логируются.

Технический долг.
Оптимизация latency, улучшение качества моделей, масштабирование под high-load.
...
Основная задача — компьютерное зрение + генеративные модели:
- построение цифрового аватара пользователя,
- наложение одежды,
- персонализированная визуализация,
- рекомендательная система образов.
