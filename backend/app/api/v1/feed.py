...
from app.schemas.feed import FeedItemResponse, FeedItemCreate, FeedItemWithStats, FeedPagination
from app.schemas.social import LikeCreate, LikeResponse
...
async def get_feed(
    ...
    for item_data in feed_with_stats:
        items.append(FeedItemWithStats(
    ...
    return FeedPagination(
        items=items,
        next_cursor=str(skip + limit) if len(items) == limit else None,
