...
from app.application.dto.social_dto import LikeCreate
...
class LikeRepository(BaseRepository[Like]):
    ...
    async def get_by_user_and_feed_item(self, db: AsyncSession, user_id: int, feed_item_id: int) -> Optional[Like]:
        result = await db.execute(select(Like).where(Like.user_id == user_id, Like.feed_item_id == feed_item_id))
        return result.scalar_one_or_none()

    async def get_user_likes(self, db: AsyncSession, user_id: int, skip: int = 0, limit: int = 50) -> List[Like]:
        result = await db.execute(select(Like).where(Like.user_id == user_id).offset(skip).limit(limit))
        return result.scalars().all()

    async def get_feed_item_likes(
        self, db: AsyncSession, feed_item_id: int, skip: int = 0, limit: int = 50
    ) -> List[Like]:
        result = await db.execute(select(Like).where(Like.feed_item_id == feed_item_id).offset(skip).limit(limit))
        return result.scalars().all()

    async def count_feed_item_likes(self, db: AsyncSession, feed_item_id: int) -> int:
        result = await db.execute(select(Like).where(Like.feed_item_id == feed_item_id))
        likes = result.scalars().all()
        return len(likes)

    async def delete_by_user_and_feed_item(self, db: AsyncSession, user_id: int, feed_item_id: int) -> bool:
        result = await db.execute(delete(Like).where(Like.user_id == user_id, Like.feed_item_id == feed_item_id))
        await db.commit()
